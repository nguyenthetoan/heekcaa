			...
			$output .= '<div class="testimonials_slider '. $class .'">';
				...
				$output .= '<ul class="testimonials_slider_ul">';
					while ($query_tm->have_posts())
					{
						$query_tm->the_post();
						$output .= '<li>';
							// first column - image from get_post_meta(get_the_ID(), 'mfn-post-link', true)
							$output .= '<div class="column mcb-column two-fifth">';
								$output .= '<div class="img-with-borders">';
									$output .= '<img class="big-border" src="'. get_post_meta(get_the_ID(), 'mfn-post-link', true) .'" />';
								$output .= '</div>';
							$output .= '</div>';

							// second column - content includes logo, rounded avatar, content
							$output .= '<div class="column mcb-column three-fifth">';
								$output .= '<div class="testimonial-main-content">';
									// top logo
									$output .= '<div class="logo">';
										$output .= '<img src="https://heekcaa.com.vn/wp-content/uploads/2017/12/logo_normal.png"/>';
									$output .= '</div>';

									// rounded author's avatar
									...
									$output .= '</div>';
                                    $output .= '<h5>@'. get_post_meta(get_the_ID(), 'mfn-post-author', true) .'</h5>';
									// testimonial content
									$output .= '<div class="testimonial-content">';
										$output .= '<div class="bq_wrapper">';
    									$output .= '<blockquote class="curly-quotes">'. get_the_content() .'</blockquote>';
      							$output .= '</div>';
									$output .= '</div>';
								$output .= '</div>';
							$output .= '</div>';
